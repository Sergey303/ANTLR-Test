...
locals[Variable varOrIRIrefCtx]
...
verb
: varOrIRIref { $verbObjectList::varOrIRIrefCtx=$varOrIRIref.p; } 
| 'a' {	
var varOrIRIrefCtx=new Variable();							   
varOrIRIrefCtx.pacElement =  TripleInt.Code("http://www.w3.org/1999/02/22-rdf-syntax-ns#type");  
varOrIRIrefCtx.graph	= new GraphIsDataProperty(){IsData=false};
$verbObjectList::varOrIRIrefCtx=varOrIRIrefCtx;	   
} ;
...
: var 
{	   
	var p = $verbObjectList::varOrIRIrefCtx;
...
|	BlankNode  { $entityCode = TripleInt.Code("blank node"); } 
...
: iRIref {$value = Expression.Constant(TripleInt.Code($iRIref.value));}
